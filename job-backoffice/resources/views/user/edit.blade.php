    ...
    <div class="w-full mx-auto p-6 bg-white rounded-lg shadow-md">
        <form method="POST" action="{{ route('user.update', ['user' => $user->id]) }}">
            ...
            <div class="mb-6 p-6 bg-gray-100 rounded-lg shadow-md">
                <h5 class="text-2xl font-semibold mb-4">Users Details</h5>


                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="title">User Name</label>
                    <span>{{ $user->name }}</span>
                </div>

                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="title">User Email</label>
                    <span>{{ $user->email }}</span>
                </div>

            <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="title">User Role</label>
                    <span>{{ $user->role}}</span>
                </div>
        ...
        <x-text-input id="owner_password" class="block mt-1 w-full pr-10 py-2 px-3"  placeholder="Enter User Password" name="owner_password"
